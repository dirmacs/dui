...
pub fn ScoreRing(
 ...
 let color = if clamped > 70 {
  ...
  "var(--dm-warning, #eab308)"
 } else {
  "var(--dm-danger, #ef4444)"
 };
 let size_str = format!("{}px", size);
 view! {
  <div class=format!("inline-flex flex-col items-center gap-1 {}", class) role="img" aria-label=format!("Score: {} out of 100{}", clamped, label.map(|l| format!(" for {}", l)).unwrap_or_default()) >
   <svg viewBox="0 0 100 100" style=format!("width: {}; height: {};", size_str, size_str) >
    // Background circle
    <circle cx="50" cy="50" r=format!("{}", radius) fill="none" stroke="var(--dm-border, #333)" stroke-width="8" opacity="0.3" />
    // Score arc
    ...
    // Score text
    <text x="50" y="50" text-anchor="middle" dominant-baseline="central" fill="var(--dm-text, #fff)" font-size="24" font-weight="bold" >
     {format!("{}", clamped)}
    </text>
   </svg>
   {label.map(|l| view! {
    <span class="text-xs font-medium" style="color: var(--dm-text-muted);" >
     {l}
    </span>
   })}
  </div>
